...
async def get_case(case_id: int, db: AsyncSession = Depends(get_db)):
    ...
    # Создаем ответ
    case_response = CaseDetailResponse.model_validate(case)
    case_response.items = items
...
async def open_case(
    ...
    try:
        ...
        # Выбираем случайный предмет на основе весов
        ...
        # Начинаем транзакцию
        # 1. Списываем звезды с баланса пользователя
...
def select_random_item(items_data: List[dict]) -> Optional[dict]:
    ...
    Args:
        items_data: Список предметов с весами
        
    Returns:
        Выбранный предмет или None
    """
    try:
        if not items_data:
            return None
